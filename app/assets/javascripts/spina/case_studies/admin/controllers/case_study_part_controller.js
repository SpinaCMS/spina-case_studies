(() => {
  ...
  application.register("case-study-part-form", class extends Stimulus.Controller {
    static get targets() {
      ...
      Sortable.create(this.listTarget, {
        ...
        onUpdate: function (event) {
          let order_of_ids = [...this.listTarget.children].map(function (item) {
            return parseInt(item.dataset.caseStudyPartId)
          })

          for (const id of order_of_ids) {
            this.contentTarget.querySelector(
              `[data-case-study-part-id='${id}'] [data-target='position']`
            ).value = order_of_ids.indexOf(id)
          }
        }.bind(this)
      })
    }
